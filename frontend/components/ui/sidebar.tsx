...
// NOTE: this app's components/ui/ scope does not include a standalone
// hooks/ directory or an input.tsx primitive, so both are inlined below
// (useIsMobile, and a plain styled <input> for SidebarInput) rather than
// pulled in as separate files.
...
function SidebarInput({ className, ...props }: React.ComponentProps<"input">) {
  return (
    <input
      ...
      data-sidebar="input"
      className={cn(
        "bg-background border-input flex h-8 w-full min-w-0 rounded-md border px-3 py-1 text-sm shadow-xs outline-none transition-[color,box-shadow] focus-visible:border-ring focus-visible:ring-ring/50 focus-visible:ring-[3px] disabled:pointer-events-none disabled:cursor-not-allowed disabled:opacity-50",
        className
      )}
      {...props}
    />
