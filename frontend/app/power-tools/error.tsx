...
export default function PowerToolsError({
  error,
  reset,
}: {
  ...
  useEffect(() => {
    ...
    // (matches the root app/error.tsx convention).
    console.error("Power Tools failed to render:", error);
  }, [error]);

  return (
    <div className="min-h-screen md:ml-64 xl:mr-72 flex items-center justify-center px-4 py-16" style={{ background: "var(--bg-primary)" }}>
      <div className="max-w-md w-full rounded-lg p-8 text-center" style={{ background: "var(--bg-card)", border: "1px solid var(--border)" }}>
        ...
        <h1 className="text-lg font-bold text-white">Power Tools hit a snag</h1>
        <p className="text-sm text-slate-400 mt-2 leading-relaxed">
          One of the AI tools failed to load or run. Your saved profile and resume are untouched — try again, or head back to the dashboard.
        </p>
        {error?.digest && (
