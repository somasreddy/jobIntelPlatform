...
export default function CareerGraphError({
  error,
  reset,
}: {
  ...
  useEffect(() => {
    console.error("Career Graph route error:", error);
  }, [error]);
  ...
  return (
    <div className="flex min-h-screen items-center justify-center px-4 py-16" style={{ background: "var(--bg-base)" }}>
      <div className="max-w-md w-full rounded-lg p-8 text-center" style={{ background: "var(--bg-card)", border: "1px solid var(--border)" }}>
        ...
        <h1 className="text-lg font-bold text-white">Career Graph couldn&apos;t load</h1>
        <p className="text-sm text-slate-400 mt-2 leading-relaxed">
          Something interrupted your health score, skills, goals, or milestones. Your data is
          safe on the server — try again, or head back to the dashboard.
        </p>
        {error?.digest && (
