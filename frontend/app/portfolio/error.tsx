...
export default function PortfolioBuilderError({
  error,
  reset,
}: {
  ...
  useEffect(() => {
    ...
    console.error("Portfolio builder page error:", error);
  }, [error]);

  return (
    <div className="flex min-h-screen items-center justify-center px-4 py-16 md:ml-64">
      <div className="max-w-md w-full rounded-lg p-8 text-center" style={{ background: "var(--bg-card)", border: "1px solid var(--border)" }}>
        ...
        <h1 className="text-lg font-bold text-white">Portfolio builder hit a snag</h1>
        <p className="text-sm text-slate-400 mt-2 leading-relaxed">
          Something went wrong loading your portfolio editor. Anything already saved is safe — try again.
        </p>
        {error?.digest && (
