...
export default function PublicPortfolioError({
  ...
  useEffect(() => {
    ...
    console.error("Public portfolio page error:", error);
  }, [error]);
  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-4 py-16" style={{ background: "var(--bg-base)" }}>
      <div className="max-w-md w-full rounded-lg p-8 text-center" style={{ background: "var(--bg-card)", border: "1px solid var(--border)" }}>
        ...
        <h1 className="text-lg font-bold text-white">This portfolio couldn&apos;t load</h1>
        <p className="text-sm text-slate-400 mt-2 leading-relaxed">
          Something went wrong while fetching this profile. It might be temporary — try again, or head back home.
        </p>
        {error?.digest && (
