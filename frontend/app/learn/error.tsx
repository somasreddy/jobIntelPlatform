...
export default function LearnError({
  ...
  useEffect(() => {
    ...
    console.error("Learning Engine failed to load:", error);
  }, [error]);
  ...
  return (
    <div className="flex min-h-screen items-center justify-center px-4 py-16" style={{ background: "var(--bg-base)" }}>
      <div className="max-w-md w-full rounded-lg p-8 text-center" style={{ background: "var(--bg-card)", border: "1px solid var(--border)" }}>
        ...
        <h1 className="text-lg font-bold text-white">Learning Engine hit a snag</h1>
        <p className="text-sm text-slate-400 mt-2 leading-relaxed">
          We couldn&apos;t load your learning paths and resources. Your saved progress and completions are untouched — try again, or head back to the dashboard.
        </p>
        {error?.digest && (
