...
export default function InterviewError({
  ...
  useEffect(() => {
    console.error("Interview Prep page error:", error);
  }, [error]);
  ...
  return (
    <div className="min-h-[70vh] flex items-center justify-center px-4 py-16" style={{ background: "var(--bg-base)" }}>
      <div className="max-w-md w-full rounded-lg p-8 text-center" style={{ background: "var(--bg-card)", border: "1px solid var(--border)" }}>
        ...
        <h1 className="text-lg font-bold text-white">Couldn&apos;t load the interview question bank</h1>
        <p className="text-sm text-slate-400 mt-2 leading-relaxed">
          Something interrupted the interview prep page. Your saved answers and scores aren&apos;t affected — try again, or head back to the dashboard.
        </p>
        {error?.digest && (
