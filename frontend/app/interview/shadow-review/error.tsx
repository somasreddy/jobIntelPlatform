...
export default function ShadowReviewError({
  error,
  reset,
}: {
  ...
  useEffect(() => {
    console.error("Shadow Interview Review page error:", error);
  }, [error]);
  ...
  return (
    <div className="min-h-[70vh] flex items-center justify-center px-4 py-16" style={{ background: "var(--bg-base)" }}>
      <div className="max-w-md w-full rounded-lg p-8 text-center" style={{ background: "var(--bg-card)", border: "1px solid var(--border)" }}>
        ...
        <h1 className="text-lg font-bold text-white">Couldn&apos;t generate your shadow review</h1>
        <p className="text-sm text-slate-400 mt-2 leading-relaxed">
          Something interrupted the debrief. Your interview notes weren&apos;t submitted — try again, or head back to the dashboard.
        </p>
        {error?.digest && (
